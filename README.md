...
# Binôme Pericat Etienne et Kaïs Ben Youssef
...
- Le test utilise d'autres opérations pour vérifier l'état des variables et ainsi vérifier les spécifications.
En particulier l'utilisation de getters et setters est indispensable pour contrôler l'état des variables tout en conservant l'encapsulation.
...
- L'ensemble des chemins couverts ne signifie pas obligatoirement que tous les cas aient été pris en
compte. Par exemple l'injection de "bugs" (injection de code ou autre faille) ou encore des valeurs innatendues, pourrait potentiellement
faire dysfonctionner le programme sans pour autant apparaître en tant que test manquant.
...
- Puisque la méthode retourne des coordonnées aléatoires il est impossible de déterminer
avant l'exécution les valeurs qui vont être attribuées aux coordonnées. Logiquement on ne pourra donc
pas créer d'oracle de vérification et ainsi faire passer un test.
